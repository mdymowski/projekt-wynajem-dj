        ...
        <?php else: ?>
            <?php foreach ($opinie as $opinia): ?>
                <div class="opinia">
                    ...
                    <a href="admin_edit_opinion.php?id_opinii=<?= $opinia['id_opinii'] ?>" class="button">Edytuj opinię</a>

                    <!-- Link do usunięcia opinii -->
                    <a href="admin_delete_opinion.php?id_opinii=<?= $opinia['id_opinii'] ?>" class="button" onclick="return confirm('Czy na pewno chcesz usunąć tę opinię?');">Usuń opinię</a>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
