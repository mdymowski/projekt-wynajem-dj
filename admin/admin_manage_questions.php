        ...
        <?php else: ?>
            <?php foreach ($questions as $question): ?>
                <div class="question">
                    ...
                    <?php if (empty($question['odpowiedz'])): ?>
                        <a href="admin_answer_question.php?id_faq=<?= $question['id_faq'] ?>" class="button">Odpowiedz na pytanie</a>
                    <?php endif; ?>

                    <!-- Link do edycji pytania -->
                    <a href="admin_edit_question.php?id_faq=<?= $question['id_faq'] ?>" class="button">Edytuj pytanie</a>

                    <!-- Link do usunięcia pytania -->
                    <a href="admin_delete_question.php?id_faq=<?= $question['id_faq'] ?>" class="button" onclick="return confirm('Czy na pewno chcesz usunąć to pytanie?');">Usuń pytanie</a>
